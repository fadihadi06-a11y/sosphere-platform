...
-- ═══════════════════════════════════════════════════════════════
-- version:  20260502220317
-- name:     foundation_canonical_identity_rpc_v3
-- live:     foundation_canonical_identity_rpc_v3
-- sha256:   13ed21d0fe9fdedb (first 16 hex of body sha256 — drift detector key)
...
-- v3: replace records with explicit scalar variables to avoid
-- 'record not yet assigned' errors when an optional join misses.
-- Each potential null is now an explicit nullable scalar.
...
BEGIN
  ...
  IF v_email IS NULL AND v_has_password IS NULL THEN
    -- Defensive: row missing entirely (delete race)
    RETURN jsonb_build_object(
  ...
  -- Active membership (one per user enforced by partial unique index)
  ...
  IF v_company_id IS NOT NULL THEN
    SELECT name, owner_id
      INTO v_company_name, v_company_owner
    ...
    IF v_company_role IN ('employee', 'dispatcher') THEN
      SELECT e.name, e.status, e.role, e.zone, e.department,
             e.phone, e.verified, e.last_seen_at
        INTO v_emp_name, v_emp_status, v_emp_role, v_emp_zone, v_emp_department,
  ...
  IF v_profile_co_id IS NOT NULL
  ...
  RETURN jsonb_build_object(
    ...
    'employee_data',   CASE WHEN v_emp_name IS NOT NULL
                          THEN jsonb_build_object(
                            ...
                            'role', v_emp_role, 'zone', v_emp_zone,
